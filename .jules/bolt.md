...
## 2024-05-17 - [N+1 query in item image update loop]
**Learning:** Checking `$model->relation()->count()` inside a `foreach` loop (e.g., when validating total uploaded files) triggers an N+1 query issue, executing a new database COUNT query for every iteration.
**Action:** Pre-calculate the count outside the loop using `$currentCount = $model->relation()->count();` and manually increment the local variable inside the loop (`$currentCount++`) when new related records are created.
