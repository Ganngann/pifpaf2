...
## 2026-07-15 - N+1 in Loop with aggregate relations
**Learning:** Calling aggregate relations like `$model->relation()->count()` inside a loop triggers a new database query on every iteration, leading to an N+1 performance bottleneck.
**Action:** Pre-calculate the count before the loop and track it with a local variable.
