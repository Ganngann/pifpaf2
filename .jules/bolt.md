...
## 2026-07-05 - [Avoid N+1 queries in loops with count()]
**Learning:** Using relations and aggregrations like `images()->count()` inside iterative structures like `foreach` loops triggers N+1 database queries, significantly impacting performance during bulk updates or uploads.
**Action:** Always pre-calculate relational counts and state values prior to the loop and use a local variable to track increments during the loop execution.
