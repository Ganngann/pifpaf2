...
class ItemController extends Controller
{
    ...
    public function createFromAi(Request $request)
    {
        ...
        // Prevent IDOR by ensuring the AI request belongs to the authenticated user
        if (!$aiRequest || $aiRequest->user_id !== Auth::id()) {
            return response()->json(['success' => false, 'message' => 'Requête IA non trouvée ou accès refusé.']);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('image_path')) {
            ...
            if (!$aiRequest || $aiRequest->user_id !== Auth::id()) {
                abort(403, 'Unauthorized or Image not found.');
            }
